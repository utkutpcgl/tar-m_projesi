Status: COMPLETE — DOCUMENT-ONLY LANE ADAPTATION
Planner depth: 0
Parent plan: (root plan)

Codex integration note — lane scope (2026-08-14)

- Exact planner commit `ee08b911a08974022e7857d9b271f4eb788e9cb3`
  was verified plan-only and integrated from its exact blob before this note.
- The durable worker contract permits writes only to this plan and
  `docs/research/SPOT_SPRAY_SENSOR_OPTICS_SURVEY_V1.md`. Therefore later
  references to a survey YAML, calculator script, result JSON, or focused test
  file are advisory and are superseded for this lane. Calculations, normalized
  inputs, dispositions, source register, and validation receipts must instead
  be self-contained in the survey; deterministic checks run ephemerally and
  do not persist additional files.
- This scope adaptation changes no hard screen, source tier, safety boundary,
  baseline golden value, challenger trigger, or final-decision rule below.

Execution receipt — Pass 2 (2026-08-14)

- Package 1 source/baseline lock is implemented in the survey with exact local
  SHA-256 identities and a no-drift check from planner base to the observed
  shared-worktree HEAD.
- Package 2's permitted subset is embedded self-contained in the survey:
  formulas, normalized baseline inputs, claim taxonomy, safety flags, source
  register, and fail-closed comparison rules. No prohibited auxiliary artifact
  was written.
- Package 3 golden validation passed ephemerally at `abs_tol=1e-12` against
  `docs/results/controlled_capture_optimization_v2.json`, including span,
  FOV/GSD, target pixels, WD envelope, blur, safe swath, and payload. Physical
  A–E remains unmeasured. Packages 4–11 remain pending.

Execution receipt — Pass 3 (2026-08-14)

- Package 4 is implemented in the survey for one same-family visible-mono
  primary, one vendor-diverse mono backup, one NIR primary, and one NIR backup.
  Each retained exact camera is bound to a sourced lens/filter/window chain;
  the NIR emitter's unresolved terminal configuration identity is exposed as
  an H1/H8 failure instead of defaulted.
- Mode A/Mode B geometry, native target support, one/two-camera payload,
  common-FOV safe swath, body count, dated component subtotals, H1–H10 status,
  and explicit trigger dispositions are self-contained in the survey. Thirteen
  ephemeral deterministic assertions passed after correcting an initial
  expected-value typo; no auxiliary artifact was written.
- Two primary-paper RGB-versus-NIR comparisons are recorded as directional,
  non-equivalent task evidence. Visible mono and RGB+NIR remain conditional
  with closed triggers; NIR-only action replacement and dominated backups are
  screened out. Packages 5–11 remain pending.

Execution receipt — Pass 4 (2026-08-14)

- Package 5 is implemented in the survey with exactly two simultaneous
  area-scan snapshot candidates: the JAI `FS-3200T-10GE-NNC` primary and XIMEA
  `MQ022HG-IM-SM5X5-7NIR2` backup. Pushbroom and sequential-filter classes
  are terminal rejects; reconstructed rasters are never counted as independent
  samples.
- Exact raw lattices, band definitions, lens/window/filter boundaries,
  trigger/interface facts, matched geometry, per-band 10/20 mm support,
  safe-swath/body count, payload, dated partial cost and H1–H10 are embedded
  self-contained. Sixty-three ephemeral assertions passed after two
  over-precise validation-fixture literals were rebound to the exact formulas;
  no auxiliary artifact was written.
- The exact-camera paired RGB paper opens JAI's task-value trigger, but H1,
  H6–H8 and physical A–E remain open, so its disposition is
  `CONDITIONAL_CHALLENGER_TASK_TRIGGER_OPEN_H6_H8_PHYSICAL_CLOSED`. XIMEA is
  `SCREENED_OUT_H3_H4_H7_H8_H9` because its 5×5 lattice yields only
  `409×217` samples per band and a theoretical six spectral bays before the
  retained RGB camera. Packages 6–11 remain pending.

Execution receipt — Pass 5 (2026-08-14)

- Package 6's thermal subset is implemented in the survey with one exact
  compact geometry primary, Boson+ `22640AS50-6IARX`, and one exact
  snapshot-timing backup, A6751 SLS `29439-251` with `4215424` lens. Both
  retain the frozen RGB action channel and use a separate exact germanium
  LWIR window; thermal-only action is a terminal safety reject.
- Tier-1 timing, lens, spectrum, interface, focus, NETD/raster and SWaP facts;
  Tier-2 task evidence; and dated public-cost boundaries are separated.
  Common-FOV geometry, native target support, body/bay count, payload and
  H1–H10 dispositions are self-contained. Twenty-four ephemeral assertions
  passed; no auxiliary artifact was written.
- Boson+ is `SCREENED_OUT_H2_H7_H8` because its exact datasheet specifies
  row-at-a-time readout without a source-bound global/effective integration
  event. A6751 is `SCREENED_OUT_H4_H7_H8`: its shortest factory lens needs
  two thermal bays and a three-body RGB+LWIR stack in the frozen envelope.
  No thermal challenger survives; the depth subset and Packages 7–11 remain
  pending.

Execution receipt — Pass 6 (2026-08-14)

- Package 6's depth subset is implemented with exactly one integrated RGB-D
  primary, Orbbec Gemini 335L `G40055-170`, and one camera-side ToF backup,
  Basler blaze-101 order `107796`. The frozen Basler RGB remains the action
  channel; passive host stereo is terminal-rejected because its depth path
  adds a host NVIDIA/CUDA workload.
- Native depth geometry at 520/590 mm, target support, analytic common-FOV
  bounds, two-body camera count, timing/trigger constraints, depth/confidence
  payload, dated partial costs, ambient/valid-pixel semantics, Tier-2 task
  evidence and H1–H10 dispositions are embedded self-contained in the survey.
  Ninety-one ephemeral deterministic assertions passed; no auxiliary artifact
  was written and no physical capture or GPU work was performed.
- Neither candidate is A/B-eligible. Orbbec lacks exact near-range p95/valid
  depth and candidate-bound confidence semantics; blaze's ±5 mm value is a
  25-frame, no-ambient, white-target average and its full multi-subframe timing
  is not source-bounded. Both are frozen as
  `CONDITIONAL_CHALLENGER_MISSING_UTILITY_THRESHOLD`. At that pass boundary,
  Packages 7–11 remained pending.

Terminal execution receipt — Pass 7 (2026-08-14)

- The survey freezes `overall_decision = RETAIN_RGB_BASELINE`, an empty
  `bounded_challenger_ab_eligible` list and `replan_required = false`. The
  result is fail-closed: no challenger simultaneously closes its decisive
  H1–H10 gates, an attributable trigger and complete-stack cost. JAI retains
  only a Tier-2 task signal; it is not hardware A/B-eligible.
- Packages 7–10 are integrated as one cross-modality H1–H10 matrix, matched
  geometry/camera-count/payload/cost matrix, exact source-key bundles,
  trigger/owner/next-step ledger, Pareto/stopping table, uncertainty register,
  source-tier audit and repeated no-readiness/no-purchase boundary.
- Package 11 is completed through the lane's documented self-contained
  adaptation: current local authority hashes, baseline V2 JSON goldens,
  modality arithmetic, table schemas, dispositions, source metadata,
  terminal rejects, empty eligibility, safety flags and allowed-file scope are
  checked ephemerally. The exact final assertion count and stdout are frozen in
  survey Section 26; no auxiliary file, GPU work or physical capture exists.

Lane-package reconciliation:

| Planner package | Lane result | Evidence location |
|---|---|---|
| 1 | `COMPLETE` | survey Sections 2–4; five pinned local identities |
| 2 | `COMPLETE_DOC_EQUIVALENT` | Sections 1, 5, 20–25; self-contained schema/formulas/sources |
| 3 | `COMPLETE` | Sections 6–7 and terminal V2 golden rerun |
| 4 | `COMPLETE` | Sections 10–12; visible mono and NIR screens |
| 5 | `COMPLETE` | Sections 13–14; snapshot multispectral screen |
| 6 | `COMPLETE` | Sections 15–18; thermal and depth screens |
| 7 | `COMPLETE` | Section 21; normalized geometry, bodies and interfaces |
| 8 | `COMPLETE` | Sections 19, 21 and 23; incomplete-cost-aware Pareto decision |
| 9 | `COMPLETE` | Section 22; attributable trigger ledger and stopping rules |
| 10 | `COMPLETE` | Sections 19–25 and 27; report/citation/uncertainty assembly |
| 11 | `COMPLETE_DOC_EQUIVALENT` | Section 26; ephemeral deterministic freeze validation |

The original auxiliary YAML/script/result/test checkboxes below are retained
as planner intent, not outstanding lane work. The top-of-plan scope note
supersedes them because this worker may write only the plan and survey.
...
Bağımlılık: Paket 1–3.
 ...
 - [x] Yalnız simultaneous area-scan snapshot adayları ara.

 - [x] Pushbroom ve sequential filter-wheel adaylarını terminal reject listesine koy.

 - [x] En fazla bir primary ve bir backup exact multispectral SKU seç.

 - [x] Raw band lattice ve independent sample count'u Tier-1 kaynaktan çıkar.

 - [x] Her decision-critical band için matched-FOV GSD ve 10/20 mm target pixel hesapla.

 - [x] Vendor reconstructed raster'ını native band sampling olarak kullanma.

 - [x] Visible color safety yeterliliğini belirle; belirsizse frozen RGB'yi additive stack'te koru.

 - [x] Lens, window, filter ve band-specific transmission alanlarını tamamla.

 - [x] Tier-2 paired RGB control evidence yoksa task-value trigger'ını kapalı bırak.
...
Bağımlılık: Paket 1–3.
 ...
 - [x] Thermal-only architecture’ı rejected olarak kaydet.

 - [x] En fazla bir primary ve bir backup industrial LWIR candidate seç.

 - [x] Thermal lens ve LWIR-compatible window’u ayrı exact parçalar olarak kaydet.

 - [x] Native raster, NETD, frame timing, integration, focus, trigger ve price alanlarını tamamla.

 - [x] RGB+thermal common FOV, camera count ve sync hesaplarını üret.

 - [x] En fazla bir integrated RGB-D ve bir on-device depth backup seç.

 - [x] Host-side stereo adaylarını rejected olarak kaydet.

 - [x] Depth accuracy/precision’i exact 520–590 mm aralığında ara.

 - [x] Ambient/sunlight, emitter wavelength, confidence semantics ve valid-pixel bilgisini kaydet.

 - [x] Downstream depth utility threshold eksikse CONDITIONAL_CHALLENGER_MISSING_UTILITY_THRESHOLD üret.

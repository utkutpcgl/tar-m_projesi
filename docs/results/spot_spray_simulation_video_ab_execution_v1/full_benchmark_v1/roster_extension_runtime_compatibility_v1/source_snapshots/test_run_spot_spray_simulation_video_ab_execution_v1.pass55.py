...
def test_live_roster_extension_release_and_combined_plan_validate() -> None:
    ...
    result = execution.validate_roster_extension_release(
        paths["extension_config"]
    )
    assert result["status"] == "READY_FOR_MANAGER_VALIDATION"
    assert result["candidate_10_started"] is False
    assert result["rendering_calls"] == 0
...
def test_live_roster_extension_seal_repeat_is_idempotent_and_render_free(
    ...
    first = execution.seal_roster_extension_release(CONFIG)
    second = execution.seal_roster_extension_release(CONFIG)
    after = {
        relative: execution.sha256_file(paths["synthetic"] / relative)
        for relative in execution._roster_extension_required_relative_files()
    }

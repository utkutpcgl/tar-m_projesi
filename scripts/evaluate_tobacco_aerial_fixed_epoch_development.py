#!/usr/bin/env python3
"""Evaluate Tobacco real-data screens at one frozen fixed epoch."""
...
TOBACCO_DATASET_ID = "tobacco_aerial"
...
def validate_training_inputs(
    config: dict[str, Any],
    data_root: Path,
    tobacco_calibration: list[SampleRecord],
    ...
    tobacco_train = [
        record for record in training if record.dataset_id == TOBACCO_DATASET_ID
    ]
    if any(record.split != "train" for record in tobacco_train):
        raise ValueError("Only frozen Tobacco training campaigns may enter training")
    exposure = weights.get(TOBACCO_DATASET_ID, 0.0)
    if bool(tobacco_train) != (exposure > 0.0):
        raise ValueError("Tobacco manifest presence and sampling exposure disagree")
    expected_known = BASE_KNOWN_CROP_IDS | ({TOBACCO_CROP_ID} if exposure > 0.0 else set())
    known_ids = {int(value) for value in config["model"]["known_crop_ids"]}
    if known_ids != expected_known:
        raise ValueError(f"Unexpected known_crop_ids: {sorted(known_ids)}")
    if tobacco_train:
        if len(tobacco_train) != 720:
            raise ValueError(f"Expected 720 Tobacco training rows, got {len(tobacco_train)}")
        if {record.target_crop_id for record in tobacco_train} != {TOBACCO_CROP_ID}:
            raise ValueError("Unexpected Tobacco crop ID in training")
    overlap = {record.group_id for record in tobacco_train} & {
        record.group_id for record in tobacco_calibration
    }
    if overlap:
        raise ValueError(f"Tobacco calibration group leaked into training: {overlap}")
    train_paths = {
        value
        for record in tobacco_train
        for value in (record.image_path, record.mask_path)
    }
    calibration_paths = {
        value
        for record in tobacco_calibration
        for value in (record.image_path, record.mask_path)
    }
    if train_paths & calibration_paths:
        raise ValueError("Tobacco calibration files leaked into training")
    return (
        training_manifest,
        exposure,
        len(tobacco_train),
        len({record.group_id for record in tobacco_train}),
...
def main() -> None:
    ...
    tobacco_calibration = evaluation_records["tobacco"]
    if len(tobacco_calibration) != 240:
        raise ValueError("Expected 240 field-balanced Tobacco calibration rows")
    if {record.target_crop_id for record in tobacco_calibration} != {TOBACCO_CROP_ID}:
        raise ValueError("Unexpected Tobacco calibration crop ID")
    ...
    for seed in seeds:
        ...
        (
            ...
            tobacco_groups,
            samples_per_epoch,
        ) = validate_training_inputs(config, data_root, tobacco_calibration)
        ...
        output_dir = run_dir / f"development_fixed_epoch{arguments.fixed_epoch}_tobacco"
        artifacts: dict[str, Any] = {}
        for name, (manifest, split, _, _) in manifests.items():
            output = output_dir / f"{name}.json"
            prior = (
                run_dir
                / f"development_fixed_epoch{arguments.fixed_epoch}_weedmap"
                / f"{name}.json"
            )
            if output.exists():
                ...
                metrics = existing_metrics(
                    output, checkpoint, manifest, len(evaluation_records[name]), expected_source_tree
                )
                artifact_path = output
                reuse_source = "current_tobacco_evaluator"
            elif name != "tobacco" and arguments.reuse_completed and prior.exists():
                metrics = existing_metrics(
                    prior, checkpoint, manifest, len(evaluation_records[name]), expected_source_tree
                )
                artifact_path = prior
                reuse_source = "compatible_prior_weedmap_evaluator"
            else:
        ...
        runs.append(
            {
                ...
                "tobacco_training_exposure": tobacco_exposure,
                "tobacco_training_rows": tobacco_rows,
                "tobacco_training_groups": tobacco_groups,
                "tobacco_external_calibration_exposure": False,
    ...
    receipt = {
        ...
        "role": "tobacco_real_data_gate_development_only",
    ...
    receipt_path = candidate_dir / (
        f"tobacco_development_fixed_epoch{arguments.fixed_epoch}_seeds_{seed_label}.json"
    )
